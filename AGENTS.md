...
### Goals
1. **Event camera data reader/parser** — decode `.raw` files containing EVT 2.0/2.1/3.0 streams
...
│   ├── evt2.md        # EVT 2.0: 32-bit non-vectorized, 34-bit timestamps
│   ├── evt21.md       # EVT 2.1: 64-bit vectorized (32-pixel groups)
│   ├── evt3.md        # EVT 3.0: 16-bit stateful vectorized, 24-bit timestamps
...
| EVT 2.0 decoding | `spec/evt2.md` | 32-bit words, 4-bit type tag at `[31:28]`, 34-bit timestamps |
| EVT 2.1 decoding | `spec/evt21.md` | 64-bit words, IMX636 half-swap caveat, 32-pixel vectorization |
| EVT 3.0 decoding | `spec/evt3.md` | 16-bit stateful decoder, 24-bit timestamps, VECT_12/VECT_8 |
...
### Container Formats
- **`.raw`** — ASCII header (`% key value\n`) + raw binary event stream (EVT 2.0/2.1/3.0)
...
- **RAW header `format` field:** `EVT3;height=720;width=1280` — primary field for selecting decoder
